...
EXPORT_SYMBOL(fts_ex_mode_switch);

//for oplus tp_interface.charger_mode_switch_status
...
// for oplus tp_interface.headset_switch
void set_ft_tp_headset_state( int attached)
{
    int ret;
    u8 buf;

    FTS_INFO("Set Tp_earjack_mode : %s", attached ? "ATTACHED" : "DETATCHED");
    buf = attached ? 1 : 0;
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = 0;
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
// WuXiaolong@BSP.TP 2024/1/23 Add for edge_mode,start
    char roate_level[] = {'0'};
    if (buf[0] == '1') {
        roate_level[0] = '0';
    } else if (buf[0] == '3') {
    ...
    buf = roate_level;
// WuXiaolong@BSP.TP 2024/1/23 Add for edge_mode,end
    if (FTS_SYSFS_ECHO_ON(buf)) {
...
static DEVICE_ATTR(roate_horizontal_level, S_IRUGO | S_IWUSR,
...
static struct attribute *fts_touch_mode_attrs[] = {
    ...
    &dev_attr_roate_horizontal_level.attr,
    NULL,
};
...
int fts_ex_mode_init_spi(struct fts_ts_data *ts_data)
{
    int ret = 0;
...
int fts_ex_mode_exit_spi(struct fts_ts_data *ts_data)
{
    sysfs_remove_group(&ts_data->dev->kobj, &fts_touch_mode_group);
    return 0;
}
