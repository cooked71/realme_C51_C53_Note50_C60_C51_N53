...
#if OPLUS_TEST
#define FT_SELF_TEST_INI                       "FT8057P_Hulkk_ScreenOn_Test.ini"
#define FT_SELF_BLACK_TEST_INI                 "FT8057P_Hulkk_ScreenOff_Test.ini"
#endif
#define false 0
#define true  1
...
printk("[TP][FTS_TS][TEST]%s:"fmt"\n",  __func__, ##args); \
} while (0)

#define FTS_TEST_FUNC_ENTER() do { \
    printk("[TP][FTS_TS][TEST]%s: Enter\n", __func__); \
} while (0)

#define FTS_TEST_FUNC_EXIT()  do { \
    printk("[TP][FTS_TS][TEST]%s: Exit(%d)\n", __func__, __LINE__); \
} while (0)

#define FTS_TEST_INFO(fmt, args...) do { \
    printk(KERN_ERR "[TP][FTS_TS/I][TEST]%s:"fmt"\n", __func__, ##args); \
} while (0)

#define FTS_TEST_ERROR(fmt, args...) do { \
    printk(KERN_ERR "[TP][FTS_TS/E][TEST]%s:"fmt"\n", __func__, ##args); \
} while (0)
